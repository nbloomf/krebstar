...
> import           Kreb.Format (display, (<+>), braceList, reflow, lineS, string)
> import           Kreb.Prop
> import           Kreb.Control.Monad.LiftIO
> import qualified Kreb.Struct.Data.FiniteMap as FM
...
> instance Fmt.Display Chaos where
>   display (Chaos k) =
>     "Chaos" <+> display k
...
>   } deriving (Eq, Show)
> 
> instance (Fmt.Display ev) => Fmt.Display (MockWorld ev) where
>   display world = "MockWorld" <+> braceList
>     [ display (_filesystem world)
>     , display (_stdio world)
>     , display (_events world)
>     , display (_ioerror world)
>     , display (_clipboard world)
>     ]
> 
...
>   } deriving (Eq, Show)
> 
> instance Fmt.Display MockFilesystem where
>   display (MockFilesystem x) =
>     "MockFilesystem" <+> display x
> 
...
>     then accept
>     else reject $
>       ( reflow "Expected filesystem to be empty but got:"
>         <> lineS <> display fs)
> 
> claimFilesystemEquals
>   :: MockWorld ev -> MockFilesystem -> Check
> claimFilesystemEquals w fs =
>   explain ("Filesystem:") $ claimEqual (_filesystem w) fs
...
>     [] -> accept
>     ls -> reject $
>       ( reflow "Expected stdout to be empty but got:"
>         <> lineS <> string (concat ls))
> 
> claimStdoutEquals
>   :: MockWorld ev -> [String] -> Check
> claimStdoutEquals w ls =
>   explain "stdout:" $ claimEqual (_stdout $ _stdio w) ls
...
>     [] -> accept
>     ls -> reject
>       ( reflow "Expected stderr to be empty but got:"
>         <> lineS <> string (concat ls))
> 
> claimStderrEquals
>   :: MockWorld ev -> [String] -> Check
> claimStderrEquals w ls =
>   explain "stderr:" $ claimEqual (_stderr $ _stdio w) ls
...
>   case _ioerror w of
>     Nothing -> accept
>     Just err -> reject $ reflow $ "Unexpected IO error: " ++ err
> 
> claimHasIOError
>   :: MockWorld ev -> String -> Check
...
>     Nothing -> reject $ reflow $ "Expecting IO error: " ++ msg
> 
> claimHasSomeIOError
>   :: MockWorld ev -> Check
...
>   case _ioerror w of
>     Just _ -> accept
>     Nothing -> reject $ reflow "Expecting some IO error"
...
>   } deriving (Eq, Show)
> 
> instance Fmt.Display MockStdio where
>   display std = "MockStdio" <+> braceList
>     [ display (_stdin std)
>     , display (_stdout std)
>     , display (_stderr std)
>     ]
...
>   } deriving (Eq, Show)
> 
> instance Fmt.Display MockClipboard where
>   display (MockClipboard x) =
>     "MockClipboard" <+> display x
> 
