...
singleQuery :: String -> RuntimeState EventId IO -> IO ()
singleQuery qs env =
...
load :: String -> IO (RuntimeState EventId IO)
load str = do
  ...
  (env :: RuntimeState EventId IO) <- do
    r <- runRuntime
      (applyDecls (EventId 0 "t") ds)
      (initRuntimeState (\_ _ -> Nothing) (const Nothing))
    case r of
      Left err -> do
        hPutStrLn stderr $ show err
...
loop :: Maybe FilePath -> RuntimeState EventId IO -> IO ()
loop path env = do
  cmd <- read
  case cmd of
