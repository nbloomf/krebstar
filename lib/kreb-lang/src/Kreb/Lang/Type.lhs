...
> import           Kreb.Format (display, (<+>), emptyDoc, parens, squote, string)
> import           Kreb.Prop
...
> 
> instance Fmt.Display Uniq where
>   display (Uniq k) = display k
> 
...
>   deriving (Show)
> 
> instance Fmt.Display Scheme where
>   display (ForAll _ ar) = display ar
> 
...
>   deriving (Eq, Show)
> 
> instance Fmt.Display Arrow where
>   display (Arrow st1 st2) =
>     display st1 <+> "->" <+> display st2
> 
...
>   deriving (Eq, Show)
> 
> instance Fmt.Display Stack where
>   display (Stack x ts) =
>     "$" <+> display x
>       <+> (foldr (<+>) emptyDoc $ map (parens . display) (reverse ts))
> 
...
>   deriving (Eq, Show)
> 
> instance Fmt.Display Type where
>   display x = case x of
>     TyCon c   -> display c
>     TyVar z   -> display z
>     TyApp u v -> display u <+> display v
>     TyArr arr -> parens (display arr)
> 
...
>   deriving (Eq, Ord, Show)
> 
> instance Fmt.Display (V u) where
>   display z = case z of
>     V str    -> string str
>     Skolem u -> "#SKOL_" <> display u
> 
...
>   deriving (Eq, Ord, Show)
> 
> instance Fmt.Display (C u) where
>   display (C str) = string str
> 
...
>   } deriving (Eq, Show)
> 
> instance Fmt.Display Vars where
>   display = string . show
> 
...
>   } deriving (Eq, Show)
> 
> instance Fmt.Display Subs where
>   display = string . show
> 
...
> instance Fmt.Display Err where
>   display z = case z of
>     AtomNotDefined a ->
>       "atom" <+> squote (display a) <+> "is not defined."
>     UnrecognizedBuiltIn x ->
>       "builtin" <+> squote (display x) <+> "is not recognized."
>     BuiltInNotDefined x ->
>       "builtin" <+> squote (display x) <+> "is not defined."
>     CannotUnifySkolem x t ->
>       "cannot unify skolem variable" <+> display x
>         <+> "with type" <+> display t
>     _ -> string $ show z
