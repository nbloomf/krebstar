...
> import           Kreb.Format (string, display, align, hsep)
...
> test_Eq_rename
>   :: forall t
>    . ( Eq t, Fmt.Display t, Arb t, Prune t
>      , Normalize t, GetVars t, Rename t )
>   => Proxy t -> TestTree
> test_Eq_rename _ =
>   testGroup "Eq rename"
>     [ krebProp "x == normalize x" $
>         \(x :: t) ->
>           claimEqual x (normalize x)
> 
>     , krebProp "x == renameBinders x" $
>         \(x :: t) ->
...
> prop_Unify_examples
>   :: forall t
>    . ( Eq t, Fmt.Display t, Unify t, Rename t, GetVars t )
>   => (t, t, Either Err Subs)
>   -> Check
...
>           claimEqual True (x <<< x)
> 
>     , krebProp "x <<< y && y <<< x --> x == y" $
>         \x y ->
>           if (x <<< y) && (y <<< x)
>             then claimEqual x y
>             else accept
> 
>     , krebProp "x <<< y && y <<< z --> x <<< y" $
>         \x y z ->
>           if (x <<< y) && (y <<< z)
>             then x <<< z
>             else True
...
>     [ krebPropCases "Generic Instance"
>       prop_GenericInstance_examples
>       [ ( "#1: !S. S -> S and !S. S -> S"
...
> test_Subsume =
>   testGroup "Subsume"
>     [ krebProp "subs (subsume x y) y <<< subs (subsume x y) x" $
>         \x y ->
