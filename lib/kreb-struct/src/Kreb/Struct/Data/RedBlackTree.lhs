...
> {-# LANGUAGE GADTs #-}
> 
> module Kreb.Struct.Data.RedBlackTree (
>     RedBlackTree()
>   , empty
>   , isEmpty
>   , member
...
> instance (Fmt.Display a) => Fmt.Display (RedBlackTree a) where
>   display (Root x) = "Root" <+> display x

> instance (Fmt.Display a) => Fmt.Display (CT n c a) where
>   display x = case x of
>     E         -> "E"
>     T c a x b -> "T"
>       <+> display c <+> display a <+> display x <+> display b
> 
> instance Fmt.Display (ValueColor c) where
>   display x = case x of
>     R  -> "R";  B  -> "B"
>     BB -> "BB"; NB -> "NB"
> 
...
>           :: forall a
>            . ( Ord a, Arb a )
>           => Sample (RedBlackTree a)
>         w = fmap fromList (arb :: Sample [a])
> 
> instance
>   ( Ord a, Prune a
